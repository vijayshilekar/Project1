...
<body>
<center><font face="Shivaji01" size ="10">Saptp~</center>
...
<br>maI <strong><%=session.getAttribute("cname")%>  </strong>vaya <strong><%=session.getAttribute("cage")%> </strong> vaYa-o vyavasaaya
 iSaxaNa mau. <strong> <%=session.getAttribute("cvileg")%> </strong>  talauka  <strong><%=session.getAttribute("ctalu")%>  </strong>thisala
<strong><%=session.getAttribute("ctalu")%> </strong> ijalha  <strong><%=session.getAttribute("cdist")%> </strong>,
 rajya maharaYT/ yaoqaIla rihvaasaI Asauna  yaavdaro d`Zpuva-k kqana krtao ik, maI  
Anausauicat jaatI ikMvaa
 baaOQd Qamaa-tirt ikMvaa  Anausauicat jaatI ivamaukt jaatI BaTkyaa jamaatI  ikMvaa [-tr maagaasavaga- ivaSaoYa maagaasavaga- ivaSaoYa maagaasap`vaga- mhNauna AaoLKlaI jaato.
